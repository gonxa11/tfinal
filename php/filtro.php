<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>filtro</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/css/bootstrap.min.css" integrity="sha384-xOolHFLEh07PJGoPkLv1IbcEPTNtaed2xpHsD9ESMhqIYd0nLMwNLD69Npy4HI+N" crossorigin="anonymous"/>
    <script src="https://cdn.jsdelivr.net/npm/jquery@3.5.1/dist/jquery.slim.min.js" integrity="sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj "crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-Fy6S3B9q64WdZWQUiU+q4/2Lc9npb8tCaSX9FK7E8HnRr0Jz8D6OP9dO5Vg3Q9ct" crossorigin="anonymous"></script>
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
    <div>
        <div class="row">
            <div class="col-md-6">
                <h2>Showing all results</h2>
            </div>
            <div class="col-md-6">
                <form method="GET">
                    <select name="filtro" id="filtro" class="hiden">
                        <option value="categoria">Categoría</option>
                    </select>
                    <input type="text" placeholder="Ingrese término de búsqueda" name="termino">
                    <input type="submit" value="Filtrar">
                </form>
            </div>
        </div>
    </div>

    <?php
    $host = "localhost";
    $user = "id20707046_eider";
    $pass = "Gonzalez19@";
    $db = "id20707046_final";
    $conn = mysqli_connect($host, $user, $pass, $db);


    // Verificar si se envió el formulario de filtro
    if (isset($_GET['filtro']) && isset($_GET['termino'])) {
        $filtro = $_GET['filtro'];
        $termino = $_GET['termino'];

        // Modificar la consulta según el filtro y el término de búsqueda
        $query = "SELECT * FROM productos2 WHERE $filtro LIKE '%$termino%'";
    } else {
        // Consulta sin filtro
        $query = "SELECT * FROM productos2";
    }

    $result = mysqli_query($conn, $query);
    ?>
    <div class="cnt-ppl">
        <div class="row">
            <?php
            while ($row = mysqli_fetch_array($result)) {
                ?>
                <div class="col-md-4">
                    <img src="<?php echo $row['imagen'];?>"/>
                    <h2><?php echo $row['nombre'];?></h2>
                    <span><?php echo $row['precio'];?></span>
                    <a href="./producto.php?id=<?php echo $row['id'];?>">ver producto</a>
                </div>
                <?php
            }
            ?>
        </div>
    </div>
</body>
</html>